...
class Routing(Layer):
    ...
    def _render_bird_sessions(self, router: Router, include_tables: bool = True):
        if router.getAttribute("__routing_bird_sessions_rendered", False):
            return
        sessions = get_bgp_sessions(router)
        if not sessions:
            return
        ensure_bird_bgp_base(router, include_tables=include_tables)
        for session in sessions:
    ...
    def render(self, emulator: Emulator):
        ...
        for ((scope, type, name), obj) in reg.getAll().items():
            ...
            if type == 'rs':
                ...
                backend = get_bgp_backend(rs_node)
                if backend == "bird":
                    self._render_bird_sessions(rs_node, include_tables=False)
                elif backend == "frr":
                    raise NotImplementedError(
                        "FRR route-server nodes are not supported yet; use BIRD route servers"
                    )
