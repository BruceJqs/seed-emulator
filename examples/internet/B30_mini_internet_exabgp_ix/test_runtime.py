...
def main() -> int:
    ...
    checks: List[Dict[str, object]] = [
        ...
        record("IPv4 default compose has no enable_ipv6", "enable_ipv6" not in compose_text, "B30 is IPv4-only by default"),
        record("ExaBGP AS180 rendered", "local-as 180" in exabgp_conf, "AS180 speaker is configured"),
        record("ExaBGP peers with AS2", "peer-as 2" in exabgp_conf, "AS2 IX peer is configured"),
        record("ExaBGP peers with AS3", "peer-as 3" in exabgp_conf, "AS3 IX peer is configured"),
        record("ExaBGP announces first prefix", "203.0.113.0/24" in exabgp_conf, "first static route rendered"),
        record("ExaBGP announces second prefix", "203.0.114.0/24" in exabgp_conf, "second static route rendered"),
    ...
    if artifact_dir:
        path = Path(artifact_dir) / "b30-generated-config.json"
