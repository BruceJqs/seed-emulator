...
def test_botnet_dga_dropper_runner_accepts_preformatted_urls():
...
def test_core_ipv6_topology_inputs_normalize_padded_and_bracketed_prefixes():
    ...
    node = emu.getRegistry().get("2", "hnode", "host")
    assert str(base.getIpv6RootPrefix()) == "2000::/12"
    assert str(explicit.getIpv6Prefix()) == "2000:0:2::/64"
    assert str(node.getInterfaces()[0].getIpv6Address()) == "2000:0:2::71"
    assert str(host.getInterfaces()[0].getIpv6Address()) == "2000:0:2::71"
    assert str(ix.getPeeringLan().getIpv6Prefix()) == "2000:8:0:100::/64"
    assert str(ix.getRouteServerNode().getInterfaces()[0].getIpv6Address()) == "2000:8:0:100::fd"
...
def test_dns_zone_server_names_match_canonical_zone_names():
    ...
    dns.install("ns-example").addZone("example.")
    dns.install("root-ns").addZone(".")

    assert dns.getZoneServerNames("example.") == ["ns-example"]
    assert dns.getZoneServerNames("example") == ["ns-example"]
    assert dns.getZoneServerNames(".") == ["root-ns"]
    assert dns.getZoneServerNames("") == ["root-ns"]


def test_dns_cache_forward_zone_falls_back_to_dual_stack_zone_servers():
    emu = Emulator()
    base = Base(enableIpv6=True)
    dns = DomainNameService()
    cache = DomainNameCachingService(autoRoot=False)
    ...
    dns.install("ns-example").addZone("example.")
    cache.install("cache").addForwardZone("example.", "missing-ns-vnode")
    emu.addBinding(Binding("ns-example", filter=Filter(asn=2, nodeName="ns"), action=Action.FIRST))
    emu.addBinding(Binding("cache", filter=Filter(asn=2, nodeName="cache"), action=Action.FIRST))
    ...
    assert 'zone "example." { type forward; forwarders { 10.2.0.53; 2000:0:2::53; }; };' in named_local
    assert "missing-ns-vnode" not in named_local
